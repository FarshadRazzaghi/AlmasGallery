...
export class CustomFieldListComponent extends _CustomFieldBaseComponent {
	...
	constructor(elementRef: ElementRef, private customFieldHttpService: CustomFieldHttpService, private routing: ActivatedRoute) {
		super(elementRef);
	}
	...
	protected override async afterViewInit(): Promise<void> {
		const list = await this.customFieldHttpService.getList();
		if (list.status) {
	...
	protected onClick = (record: CustomFieldUpsert): void => {
		console.log(record);
	}
}
