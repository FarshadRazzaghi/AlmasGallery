	...
	protected override afterViewInit(): void {
		setTimeout(async () => {
			...
			if ((this.productCategoryId || this.productCategoryId == 0) && this.operation === 'Update') {
				const productCategory = await this.productCategoryService.httpService.getProductCategoryById(this.productCategoryId);
				if (!productCategory || !productCategory.data) {
					// TODO - REDIRECT TO 404 NOT FOUND PAGE;
					throw new FrWidgetError("PAGE NOT  FOUND");
				}

				// TODO - convertToModel should be in the service
		...
		setTimeout(async () => {
			...
			if (formSubmitResults.length > 0) {
				...
				if (canSubmit) {
					...
					// TODO - convertToRequest should be in the service
					//convertToRequest(this.productCategoryService.productCategory);

					if (this.operation === 'AddNew') {
						httpResponse = await this.productCategoryService.httpService.createProductCategory(requestModel);
					}
					else {
						httpResponse = await this.productCategoryService.httpService.updateProductCategoryById(this.productCategoryId ?? 0, requestModel);
					}
