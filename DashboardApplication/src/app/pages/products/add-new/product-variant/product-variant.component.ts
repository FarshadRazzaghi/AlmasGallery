...
export class ProductVariantComponent extends _AddNewBaseComponent {

  @ViewChildren(FrForm.FrFormComponent) set forms(forms: QueryList<FrForm.FrFormComponent<ProductVariant>>) {
    if (forms) {
  ...
  private _forms: FrForm.FrFormComponent<ProductVariant>[] = [];
  ...
  protected override afterViewInit(): void {
    ...
    if (variants.length === this._forms.length) {
      this._forms.map(async (form, index) => {
        ...
        await form.setModel(<ProductVariant>formValue);
      })
    }
  }
