...
export class CustomFieldUpsertComponent extends _CustomFieldUpsertBaseComponent {
  ...
  protected get options(): CustomFieldUpsertOption[] {
    const options = (this.customField.options ?? []).map(option => {
      option.dataTypeName = this.dataTypeResource.find(x => x.value == option.dataType)?.key || '';
      return option;
    });
  ...
  protected customFieldLoaded: boolean = false;
  protected deleteModalId: string = 'custom-field-delete';
  ...
  protected onOptionSelect = (option: CustomFieldUpsertOption): void => {
    this.customFieldService.updateCustomField(option);
  }
