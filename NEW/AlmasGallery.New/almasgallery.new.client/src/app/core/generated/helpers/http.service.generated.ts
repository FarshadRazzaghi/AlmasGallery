...
import { HttpAuthService } from "./http-auth.service.generated";
...
/**
 * Configuration options for "POST", "PUT", "PATCH" and "DELETE" API requests
 * @interface RequestOptions
 * @property {boolean} [cache=false] - Enable/disable response caching (GET requests only)
 * @property {number} [timeout=30000] - Request timeout in milliseconds
 * @property {number} [retries=0] - Number of retry attempts for failed requests
 */
...
export abstract class HttpServiceGeneric {
	 ...
	 */
	protected get<Response>(url: string, parameters?: string[], options: RequestOptions = DEFAULT_OPTIONS): Observable<HttpResponse<Response>> {
	 ...
	 * @returns {Observable<HttpResponse<Request>>} The HTTP response observable
	 */
	protected post<Request, Response>(url: string, data?: Request, parameters?: string[], options: RequestOptions = NO_OPTIONS): Observable<HttpResponse<Response>> {
		const apiURL = this.generateApiUrl(url, parameters);
	 ...
	 * @returns {Observable<HttpResponse<Request>>} The HTTP response observable
	 */
	protected put<Request, Response>(url: string, data?: Request, parameters?: string[], options: RequestOptions = NO_OPTIONS): Observable<HttpResponse<Response>> {
		const apiURL = this.generateApiUrl(url, parameters);
		return this.handleRequest(
	 ...
	 * @returns {Observable<HttpResponse<Request>>} The HTTP response observable
	 */
	protected patch<Request, Response>(url: string, data?: Request, parameters?: string[], options: RequestOptions = NO_OPTIONS): Observable<HttpResponse<Response>> {
		const apiURL = this.generateApiUrl(url, parameters);
		return this.handleRequest(
	 ...
	 * @returns {Observable<HttpResponse<Request>>} The HTTP response observable
	 */
	protected delete<Response>(url: string, parameters?: string[], options: RequestOptions = NO_OPTIONS): Observable<HttpResponse<Response>> {
		const apiURL = this.generateApiUrl(url, parameters);
		return this.handleRequest(
